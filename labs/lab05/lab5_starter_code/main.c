 ...
 *   - Alarm fires within microseconds of detection (well under 80ms budget)
 *   - Logging at 1 Hz normal, 2 Hz warning/critical, immediate on state transitions
 *
 * Instrumentation (cycle-accurate):
 *   - sched:  scheduling delay (sensor post → processing start)
 *   - proc:   threshold evaluation time
 *   - alarm:  alarm firing latency (sensor post → GPIO write)
 */
 ...
 * Stamps cycle count before posting for latency measurement.
 * No mutex, no UART.
 * ============================================================ */
...
void processing_task_fn(void *p1, void *p2, void *p3) {
    ...
    while (1) {
        ...
        /* Scheduling delay: sensor post → processing start */
        ...
        uint32_t alarm_latency_us = 0;
        if (new_state == STATE_CRITICAL || new_state == STATE_SENSOR_FAULT) {
            alarm_on();
            /* Alarm latency: sensor post → GPIO write complete */
            alarm_latency_us = k_cyc_to_us_floor32(k_cycle_get_32() - msg.sensor_cycles);
        ...
        /* Processing time: threshold eval + alarm GPIO */
        uint32_t proc_end = k_cycle_get_32();
        uint32_t proc_time_us = k_cyc_to_us_floor32(proc_end - proc_start);
...
void logging_task_fn(void *p1, void *p2, void *p3) {
    ...
    while (1) {
        ...
        /* Snapshot under mutex — microseconds */
        k_mutex_lock(&log_mutex, K_FOREVER);
        vitals_msg_t snap = log_vitals;
        system_state_t snap_state = log_state;
        uint32_t sched_us = log_sched_delay_us;
        uint32_t proc_us = log_proc_time_us;
        uint32_t alarm_us = log_alarm_latency_us;
        k_mutex_unlock(&log_mutex);
